    ...
    <div className="min-h-[calc(100vh-73px)] bg-surface flex">
      ...
      <div className="flex-1 flex flex-col justify-center px-8 sm:px-16 lg:px-24 max-w-2xl mx-auto w-full py-12">
        ...
        <div className="w-full">
          <h2 className="text-4xl font-bold font-editorial mb-3 text-ink">Create an Account</h2>
          <p className="text-slate mb-10 text-lg">Join the platform and discover local frontier roles.</p>
          
          <form onSubmit={handleSignup} className="space-y-6">
            <div>
              <label className="block text-sm font-medium text-ink mb-2">Full Name</label>
              <input required type="text" placeholder="Jane Doe" className="text-input w-full py-3 px-4 text-base bg-white" value={name} onChange={e => setName(e.target.value)} />
            </div>
            <div>
          ...
          <div className="mt-10 text-center text-slate">
            ...
            <Link href="/login" className="text-primary font-semibold hover:text-primary-deep transition-colors">
              Sign in
            </Link>
          </div>
        </div>
      ...
      <div className="hidden lg:flex flex-1 relative overflow-hidden bg-ink">
        ...
        <div className="absolute bottom-24 left-20 right-20 text-white">
          <h3 className="font-editorial text-[42px] mb-6 leading-[1.1] tracking-tight">Your next opportunity is closer than you think.</h3>
          <p className="text-xl text-white/70 max-w-md">Join OfficeNextDoor and connect with the best local teams.</p>
        </div>
      </div>
    </div>
  );
